...
function get_teas(){
	$.ajax({
		url: "api/products/read_teas.php",
		type: "GET",
		dataType: "json",
		...
		error: function(e){
			console.log("Error occured while getting all teas "+e);
			$.notify("Error occured while getting teas", "error");
		}
	})
}

function show_teas(teas){

	var table = document.getElementById("MyTable");
	var cpt = 0;

	for(i=0; i<teas.length; i++){
		if(cpt == teas.length) break;
		//Get first product of each row
		var previousProduct = teas[2*i];
		cpt++; 
		var tr = document.createElement("tr");
		var td1 = build(previousProduct); tr.append(td1);
		
		if(2*i+1 <teas.length){
			//Get second product of each row
			var nextProduct = teas[2*i+1];
			cpt++; 
...
function build(product){
	...
	addToCart_button.value = product.type;
